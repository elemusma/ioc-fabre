...
if ( $post->post_parent ) {
    echo '<section><a href="' . get_permalink( $post->post_parent ) . '" class="btn bg-accent-secondary text-white position-fixed text-uppercase h1 btn-go-back z-3" style="left:25px;">Go Back to ' . get_the_title( $post->post_parent ) . '</a></section>';
    }
...
if($image) {
echo wp_get_attachment_image($image['id'],'full',"",['class'=>'w-100 h-100 position-absolute d-md-block d-none','style'=>'top:0;left:0;object-fit:cover;']);
echo wp_get_attachment_image($image['id'],'full',"",['class'=>'w-100 h-100 position-relative d-md-none d-block','style'=>'']);
...
the_post_thumbnail('full',array('class'=>'w-100 h-100 position-relative d-md-none d-block','style'=>''));
} else {
$globalPlaceholderImg = get_field('global_image','options');
echo wp_get_attachment_image($globalPlaceholderImg['id'],'full',"",['class'=>'w-100 h-100 position-absolute d-md-block d-none','style'=>'top:0;left:0;object-fit:cover;']);
echo wp_get_attachment_image($globalPlaceholderImg['id'],'full',"",['class'=>'w-100 h-100 position-relative d-md-none d-block','style'=>'']);
} 
?>
</div>
</div>
